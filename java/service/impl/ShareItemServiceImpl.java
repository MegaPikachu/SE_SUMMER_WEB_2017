...
public class ShareItemServiceImpl implements ShareItemService {
	...
	public List<SelectedShare> getPrivateAll(int pagenum, int size, int uid){  
		return shareItemDao.getPrivateShareItems(pagenum, size, uid);
	}
	...
	public void delete(ShareItem shareItem){ 
		shareItemDao.delete(shareItem);
	}
	
	public void addComment(String sid) {
		ShareItem sItem = shareItemDao.getShareItemById(sid);
		int comment = sItem.getComment()+1;
		sItem.setComment(comment);
		shareItemDao.update(sItem);
	}
	...
	public String browse(String sid) {
		...
		if (shareItem == null) {
			...
			shareItem.setBrowse(shareItem.getBrowse()+1);
			shareItemDao.update(shareItem);
			return "success";
		}
	...
	public ShareItem getShareItem(String sid) {
		return shareItemDao.getShareItemById(sid);
	}
}
