...
public class ShareItemDaoImpl extends HibernateDaoSupport implements ShareItemDao {
	...
	public List<SelectedShare> getPublicShareItems(int pagenum, int size) {
		List<SelectedShare> result = new ArrayList<SelectedShare>();		
		try {
			conn();
			rowset.setCommand("{call getshare(?,?)}");
			rowset.setInt(1, pagenum);
			rowset.setInt(2, size);
			rowset.execute();
			rowset.beforeFirst();
			while (rowset.next()) {
				SelectedShare thisshare = new SelectedShare(rowset.getString(1), rowset.getInt(2), rowset.getString(3), rowset.getString(4), rowset.getInt(5), rowset.getDate(6), rowset.getDate(7), rowset.getInt(8),rowset.getInt(9), rowset.getInt(10), rowset.getString(11),rowset.getInt(12));
				result.add(thisshare);
	...
	public List<SelectedShare> getPrivateShareItems(int pagenum, int size, int uid) {
		Session session = getSession();
		SQLQuery query = session.createSQLQuery("call getprivateshare(?,?,?)");
		query.setInteger(0, pagenum);
		query.setInteger(1, size);
		query.setInteger(2, uid);
		query.executeUpdate();
