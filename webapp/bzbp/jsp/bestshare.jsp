		...
		<div class="navbar-default sidebar" role="navigation">
			<div class="sidebar-nav navbar-collapse">
				<ul class="nav" id="side-menu">
					...
					<li><a href="statistic_get"><i class="fa fa-book fa-fw"></i>
							查看统计分析</a></li>
		...
		$.ajax({
			...
			success: function(data){
				...
				html= "<table class='table table-striped table-bordered table-hover'>"+
				   ...
				   +"<th>开始时间</th><th>结束时间</th><th>点赞数</th><th>浏览数</th><th>评论数</th><th>美文或小诗</th><th></th><th></th></tr></thead><tbody>";
				...
				if(size>0){
					for (var i = 0; i < size; ++i){
						...
						html+="<td>"+getTime(data[i][5])+"</td><td>"+getTime(data[i][6])+"</td><td>"+data[i][7]+"</td><td>"+data[i][8]+"</td><td>"+data[i][9]+"</td><td>"+data[i][10]+"</td>";
						html+="<td><button onclick=\"test('"+data[i][0]+"')\">设置它为最佳的分享</button></td>";
						html+="<td><button onclick=\"comment('"+data[i][0]+"')\" data-toggle=\"modal\" data-target=\"#myModal\">查看它所有的评论</button></td></tr>";
					}
	...
	function comment(sid){
		...
		jQuery.ajax({
			...
			success : function(data) {
				...
				$('#allcomments').empty();
				html= "<table class='table table-striped table-bordered table-hover'>"+
				   "<thead><tr><th>评论者</th><th>评论内容</th></tr></thead><tbody>";
				var size = data.length;
				if(size>0){
					for (var i = 0; i < size; ++i){
						if (data[i].unameb==''){
							html+="<tr><td>"+data[i].unamea+"</td><td>"+data[i].comment+"</td></tr>";
						}
						else{
							html+="<tr><td>"+data[i].unamea+" 回复 "+data[i].unameb+"</td><td>"+data[i].comment+"</td></tr>";
